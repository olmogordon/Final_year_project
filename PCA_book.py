...
def plot_single_pca(a,b):
    ...
    #plt.xlim([-1, 1])
    #plt.ylim([-1, 1])
    ax1.scatter(a, b, s = 15)
    plt.show()
    
#function to plot progression of 2D PCA values 
def pca_plotter(n_1, n_2, H, h_final_mean, C, trial_n, freq, text_final):
    #colours = ["black", "silver", "indianred", "orangered", "darkorange", "gold", "olive", "lightgreen", "darkgreen", "aqua", "teal", "dodgerblue", "royalblue", "mediumpurple", "indigo", "fuchsia"]
    fig = plt.figure()
    ax1 = fig.add_subplot(111)
    plt.xlabel("PC1", fontsize=13)
    ...
    #plt.xlim([-1, 1])
    #plt.ylim([-1, 1])
    for i in range(0,(len(text_final)//freq)-1):
...
def H(X, n_1, n_2, n_3, lamda, lower, upper, w_1, w_2, trial_n, text_final, index_map, freq_words, freq):
    ...
    H = np.zeros((len(text_final)//freq,n_2,n_1))
...
h_initial = h_initial - h_final_mean.reshape(n_2,1)*np.ones((1,n_1))
...
pca_plotter(n_1, n_2, H, h_final_mean, C, trial_n, freq, text_final)
